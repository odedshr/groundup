...
describe('DOMinion', () => {
  ...
  describe('DOMinion.map', () => {
    ...
    it('should map a complicated tree', () => {
      let tree1 = parser.parseFromString('<ul><li><div>foo</div></li><li><div>bar</div></li></ul>');

      assert.equal(map(tree1), 'root,root>ul[0],root>ul[0]>li[1],div=bar,div=bar>Text[0],root>ul[0]>li[0],div=foo,div=foo>Text[0]');
    });

    it('should map an complicated tree', () => {
      let tree1 = parser.parseFromString('<ul id="list"><li name="item"><a href="yy"><img src="xx" /></a></li><li><div>item 2</div></li></ul>');
      assert.equal(map(tree1), 'root,ul#list,ul#list>li[1],div=item2,div=item2>Text[0],li|name:item,a|href:yy,img|src:xx');
    });
  });

  describe('DOMinion.update', () => {
    it('should replace a tree entirely', () => {
      let tree1 = parser.parseFromString('<ul><li>item 1</li><li>item 2</li></ul>'),
          tree2 = parser.parseFromString('<ul><li>item 3</li><li>item 4</li></ul>');
      
      DOM.update(tree1, tree2);

      assert.equal(serializer.serializeToString(tree1), '<ul><li>item 3</li><li>item 4</li></ul>');
    });
  
    it('should append a tree', () => {
      try {
        let tree1 = parser.parseFromString('<ul><li>item 1</li></ul>'),
            tree2 = parser.parseFromString('<ul><li>item 1</li><li>item 2</li></ul>'),
            node = tree1.childNodes[0].childNodes[0];

        DOM.update(tree1, tree2);

        assert.equal(serializer.serializeToString(tree1), '<ul><li>item 1</li><li>item 2</li></ul>');
    ...
    it('should add in the middle', () => {
      try {
        let tree1 = parser.parseFromString('<ul><li>item 1</li><li>item 3</li></ul>'),
            tree2 = parser.parseFromString('<ul><li>item 1</li><li>item 2</li><li>item 3</li></ul>'),
            node = tree1.firstChild.lastChild;

        DOM.update(tree1, tree2);

        assert.equal(serializer.serializeToString(tree1), '<ul><li>item 1</li><li>item 2</li><li>item 3</li></ul>');
