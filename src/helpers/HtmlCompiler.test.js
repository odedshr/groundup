const assert = require('assert'),
  ...
  HtmlCompiler = require('../../bin/HtmlCompiler.js'),
...
describe('html compiler', () => {
  ...
  describe('constructor', () => {
    ...
    it('should create a new compiler', () => {
      let html = new HtmlCompiler(new xmldom.DOMParser(), new xmldom.XMLSerializer());
      assert.equal(JSON.stringify(Object.keys(html)), JSON.stringify(['domParser', 'xmlSerializer']));
    });
  });
  ...
  describe('html.compileToString', () => {
    ...
    it('return the right template', () => {
      let result = html.compileToString({template : '<div>my template</div>'},{},{},'{{.:\'template\'}}');
      assert.equal(result, '<div>my template</div>');
    });

    it('return throw an Error if not found', () => {
      assert.throws(() => (html.compileToString({template : '<div>my template</div>'},{},{},'{{.:\'template2\'}}')), Error);
    });
  });

  describe('html.compile transformations', () => {
    it('should return translated texts', () => {
      let result = html.compileToString({},{ hello: 'hola'},{},'{{#hello}} world');
      assert.equal(result, 'hola world');
    });

    it('should return a variable', () => {
      let result = html.compileToString({},{},{ name: 'world'},'Hello {{name}}');
    ...
    it('should use conditionals', () => {
      ...
      assert.equal(html.compileToString({},{},data ,'my name is {{?showName}}{{name}}{{/showName}}'), 'my name is what');

      data.showName = false;
      assert.equal(html.compileToString({},{},data ,'my name is {{?showName}}{{name}}{{/showName}}'), 'my name is ');
    });
